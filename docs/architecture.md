...
| `cloud` | Roblox Open Cloud for the open place: data stores, ordered and memory stores, MessagingService publish, place publishing, asset upload and lifecycle (update in place, versions, rollback, archive), server-side Luau, the Instance API on the published place, bans, notifications, and reads about the universe, its owner, groups, inventories, subscriptions and the API key itself. Ids default from the session's `hello`/`hb` identity (`game.GameId` / `PlaceId` / `CreatorType` / `CreatorId`); the key lives on disk, never in the bridge ([cloud.md](cloud.md)). |
...
- **Open Cloud.** `cloud` uses the identity the hub reports in its heartbeats (`universeId`, `creatorType`, `creatorId`; `placeId` from `hello`), so an agent needs no ids for the open place. What the key may do comes from one table (`bridge/src/cloud/capabilities.ts`) read by both the 403 messages and `info what:"key"`, which judges it against the key's own scopes from Roblox's key introspection endpoint and falls back to trial reads against reserved names — [cloud.md](cloud.md).
...
| **Cloud surface (2026-09-14)** | offline-tested against a fake Open Cloud; key introspection verified live, the other surfaces pending ([live-test-results.md](live-test-results.md)) | `cloud` grows from six actions to twelve: `publish` (a place file becomes the live version), `memory` (sorted maps and queues), `asset` (get, update in place, versions, rollback, archive, restore), `instance` (Instance API on the published place), `restriction` (bans), `notify`, and `info` `key` / `memberships` / `roles` / `inventory` / `subscription`; the key capability report (introspection first, trial reads as fallback); every endpoint checked against Roblox's OpenAPI spec |
...
- **Open Cloud calls left out** — memory store `flush` (it wipes every map and queue in the universe), group writes, place metadata and version notes ([cloud.md §6](cloud.md#6-deliberately-not-exposed)). Not in Open Cloud at all: hash maps, a sorted-map increment, creating a place, deleting an asset, creating or deleting instances.
